...
const CymaticsCanvas = forwardRef(function CymaticsCanvas({
  ...
  // ── Field evaluation helpers (branch by plate shape + layers) ──
  function fieldValue(x, y, cn, cm, cz) {
    const ls = layersRef.current;
    const shape = plateRef.current;
    if (ls && ls.length > 1) {
      return multiValue(x, y, ls, shape, cz);
    }
    if (shape === "circular") {
      return besselValue(x, y, cn, cm, cz);
    }
  ...
  function fieldGradient(x, y, cn, cm, cz) {
    ...
    const shape = plateRef.current;
    if (ls && ls.length > 1) {
      return multiGradient(x, y, ls, shape, cz);
    }
    if (shape === "circular") {
      return besselGradient(x, y, cn, cm, cz);
    }
  ...
  useEffect(() => {
    ...
    if (!canvas || !onZoomChange) return;
    let lastDist = 0;
    const getDistance = (t) => Math.sqrt((t[0].clientX - t[1].clientX) ** 2 + (t[0].clientY - t[1].clientY) ** 2);
    const handleTouchStart = (e) => { if (e.touches.length === 2) lastDist = getDistance(e.touches); };
    const handleTouchMove = (e) => {
      if (e.touches.length === 2) {
        e.preventDefault();
        const dist = getDistance(e.touches);
        const newZoom = Math.max(0.5, Math.min(5, zoomRef.current + (dist - lastDist) * 0.005));
        onZoomChange(Math.round(newZoom * 10) / 10);
        lastDist = dist;
      }
    };
    ...
    return () => { canvas.removeEventListener("touchstart", handleTouchStart); canvas.removeEventListener("touchmove", handleTouchMove); };
  }, [onZoomChange]);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        const p = particles[i];
        const val = fieldValue(p.x, p.y, cn, cm, cz);
        const [gx, gy] = fieldGradient(p.x, p.y, cn, cm, cz);
        ...
        // Boundary: circular or rectangular
        if (isCirc) {
          const dx = p.x - HALF;
          const dy = p.y - HALF;
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        // Skip particles outside circle for clean rendering
        if (isCirc) {
          const dx = p.x - HALF;
          const dy = p.y - HALF;
        ...
        const val = Math.abs(fieldValue(p.x, p.y, nRef.current, mRef.current, zoomRef.current));
      ...
      if (isCirc) {
        ...
        ctx.strokeStyle = `hsla(${hueBase}, 60%, 40%, 0.15)`;
        ctx.lineWidth = 1;
        ctx.stroke();
