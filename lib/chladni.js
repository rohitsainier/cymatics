...
export function chladniValue(x, y, n, m, sign = 1) {
  const nx = (n * Math.PI * x) / PLATE_SIZE;
  const ny = (n * Math.PI * y) / PLATE_SIZE;
  const mx = (m * Math.PI * x) / PLATE_SIZE;
  const my = (m * Math.PI * y) / PLATE_SIZE;
  return Math.cos(nx) * Math.cos(my) + sign * Math.cos(mx) * Math.cos(ny);
}

export function chladniGradient(x, y, n, m, sign = 1) {
  const nx = (n * Math.PI * x) / PLATE_SIZE;
  const ny = (n * Math.PI * y) / PLATE_SIZE;
  const mx = (m * Math.PI * x) / PLATE_SIZE;
  const my = (m * Math.PI * y) / PLATE_SIZE;
  const np = (n * Math.PI) / PLATE_SIZE;
  const mp = (m * Math.PI) / PLATE_SIZE;
  const dx =
